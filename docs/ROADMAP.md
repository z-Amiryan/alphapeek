...
for Crypto Twitter — *not* a minute-zero launch-sniper. Brand-new tokens take a few hours to
propagate to any data index (see SPEC § 9), so the very newest return `unknown` until indexed. Lean
the value on **CoinStats**-powered Base/BNB trending tokens + wallet inspection (X's native cashtags
don't do wallets). Detection verified working across both chains; BSC required slug fixes (see SPEC § 4).
...
- **DexScreener supplementary coverage (free, keyless, zero-credit).** **CoinStats-first:** for a
  brand-new token still propagating to data indexes (detect→null + empty wallet), the Worker
  additionally taps DexScreener (`api.dexscreener.com`), extending breadth across the three
  newest-token scenarios: tokens still propagating to indexes, the long tail, and **wrong-chain
  inference** (the chosen pair's `chainId` is authoritative, so a bad chain-guess can't hide a
  token). It only runs when CoinStats returns no data, so it adds zero CoinStats credits; GoPlus
  safety then runs on the authoritative chain. A `MIN_LIQUIDITY_USD` floor + silent
  degrade-to-`unknown` keep it honest. See SPEC §4.
- **$TICKER (cashtag) detection.** A preloaded top-1000-by-rank whitelist
  (`Map<UPPER_SYMBOL, coinId>`, ≈936 symbols, generated by `scripts/build-tickers.mjs`) gates
...
- **$TICKER long-tail — coverage extensions beyond the shipped EVM single-match cut.** The
  ...
  **Solana** majority of degen cashtags (base58 detection + GoPlus-Solana safety — shipped in v0.3)
  and brand-new micro-caps not yet propagated to any data index. A naive "resolve to the highest-mcap match" is
